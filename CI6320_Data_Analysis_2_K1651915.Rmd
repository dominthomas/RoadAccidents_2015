...
visualisations are included, where necesarry, in this report. These code blocks can be recognised, as
...
Let's now see date range of our data:
\bigskip
```{r echo=TRUE}
library(lubridate)
data %>% 
...
Please note that all visualisations are embedded into this PDF Document as Scalable Vector Graphics
...
\
* Create two histograms with repsect to age, one for male and and another for female, and overlay the plots
...
Are there particular districts where accidents are more likely to occur?
...
As you can see Birmingham had the highest amount of accidents compared to any other local district.
But why isn't London at the top? Well London is a Metropolis, made up of several local districts.
...
\newpage
From the above visualisation, we can see that most accidents occur at night with street lights on, in
...
\
* Create the weekday bar plot by taking the frequency of each frequecy into account.
\
...
From the above visualisation, we can conclude that Friday had the most number of accidents followed by
...
\bigskip
```{r fig.cap="Seasonality and Journey Purpose of drivers with the top 6 deadliest days", echo = FALSE, fig.align='center',fig.height = 10, fig.width = 8}
require(gridExtra)
...
\newpage
Figure 6 shows 'Journey as part of work' had the highest number of accidents with clear seasonal peaks
around September and January, when schools open and right after the New Year holiday. The second bar 
plot shows that the deadliest day of that year was the 10th of February, followed by the 11th of May.
...
What type of roads are accidents more likely to occur? Does speed limit have an effect on the number
...
and another displaying the top 5 road types with the highest amount of accidents in the UK.
...
\
* Substitute road type IDs with their corresponing values in the Data Dictionary
\
...
From Figure 7, we can conclude that accidents are more likely to occur in 30mph zones.
A roads in the UK account for the highest number of accidents and single carriageways have the highest 
number of accidents.
...
\newpage
From Figure 8, we can clearly see that 'Going ahead other' caused the highest number of accidents.
...
```
\bigskip
* And finally substitute each vehicly type ID with its corresponding value from the Data Dictionary.
...
So in this instance, Motorcycles over 500cc and Motorcycles less than or eqaul to 125cc are in the same
...
Another interesting observation is how similar Goods Vehicles over 7.5 tonnes are to motorcyles over 
500cc, there is a common saying motorcylists and large goods vehicles are a dangerous combination, 
...
However, from the third visualisation onwards, things started to ease, as familiarity with R started to 
...
Overall, it has taken 44 hours to create this document.

One of the major advantages of using R over Tableau is that it gives more room for automation, as R is a
programming language.
But it also has many powerful libraries which will allow anyone to create powerful complex graphs that 
...
Using the \textbf{dplyr} to clean, filter and transform data was very intuitive, and this allowed for 
...
Overall, doing this coursework in R has become a worthwhile investment, as the skills learnt in R 
